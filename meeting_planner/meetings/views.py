...
@login_required #declarater to ensure this view is enabled only after successful login
def detail(request, id):
    # meeting = Meeting.objects.get(pk=id)
    meeting = get_object_or_404(
        Meeting, pk=id
    )  # Takes in model name and the input parameter to check in database
...
def new(request):
    if request.method == "POST":
        form = MeetingForm(
            request.POST
        )  # passing the request object to modelform_factory
...
def edit(request, id):
    ...
    if request.method == "POST":
        form = MeetingForm(
            request.POST, instance=meeting
        )  # passing the request object to modelform_factory along with the meeting object instance that needs to be edited
        if form.is_valid():  # server validation for form inputs
            form.save()
